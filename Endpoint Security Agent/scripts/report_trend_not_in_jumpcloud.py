"""
Cron script: endpoints en Trend Vision One que NO están en JumpCloud.
...
REPORT_KEY  = "trend_not_in_jumpcloud"
...
def fetch_trend_endpoints(settings) -> list[dict]:
...
def fetch_jumpcloud_hostnames(settings) -> set[str]:
    import httpx
    api_key = settings.jumpcloud_api_key
    ...
    while True:
        ...
        for s in items:
            ...
            if hn:
                hostnames.add(normalize(hn))
            if dn and dn != hn:
                hostnames.add(normalize(dn))
        if len(items) < 100:
            break
        skip += 100
    return hostnames
...
def fetch_data(settings) -> list[dict]:
    ...
    trend_endpoints = fetch_trend_endpoints(settings)
    print(f"  Trend: {len(trend_endpoints)} endpoints")

    print("Consultando JumpCloud...")
    jc_hostnames = fetch_jumpcloud_hostnames(settings)
    print(f"  JumpCloud: {len(jc_hostnames)} dispositivos")

    missing = [e for e in trend_endpoints if e["hostname_key"] not in jc_hostnames]
...
def build_excel(rows: list[dict], output_path: Path) -> None:
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Trend sin JumpCloud"

    now_str = datetime.now().strftime("%d/%m/%Y %H:%M")

    ws.merge_cells("A1:K1")
    t = ws["A1"]
    t.value = "Trend Vision One — Endpoints no encontrados en JumpCloud"
    ...
    s = ws["A2"]
    s.value = f"Generado: {now_str}  |  Datos en tiempo real  |  Total: {len(rows)} endpoints"
    ...
    for i, row in enumerate(rows, 1):
        ...
        vals = [i, row["hostname"], row["os"], row["platform"], row["ip"],
                row["last_seen"], row["days_offline"] if row["days_offline"] is not None else "—",
                row["last_user"], row["edr_status"], row["epp_status"], row["serial"]]
...
def main() -> None:
    ...
    print(f"Encontrados: {len(rows)} endpoints en Trend que NO están en JumpCloud")
