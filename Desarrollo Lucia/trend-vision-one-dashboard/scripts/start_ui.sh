#!/usr/bin/env bash
set -euo pipefail

read_env_var() {
  local key="$1"
  local default_value="$2"
  local value=""

  if [[ -f .env ]]; then
    value="$(awk -F= -v k="$key" '$0 ~ ("^" k "=") {sub(/^[^=]*=/, "", $0); print $0; exit}' .env)"
    value="${value%$'\r'}"
    value="${value%\"}"
    value="${value#\"}"
    value="${value%\'}"
    value="${value#\'}"
  fi

  if [[ -n "$value" ]]; then
    printf "%s" "$value"
  else
    printf "%s" "$default_value"
  fi
}

BACKEND_HOST="${APP_BACKEND_HOST:-$(read_env_var APP_BACKEND_HOST 127.0.0.1)}"
BACKEND_PORT="${APP_BACKEND_PORT:-$(read_env_var APP_BACKEND_PORT 8000)}"
STREAMLIT_HOST="${APP_STREAMLIT_HOST:-$(read_env_var APP_STREAMLIT_HOST 127.0.0.1)}"
STREAMLIT_PORT="${APP_STREAMLIT_PORT:-$(read_env_var APP_STREAMLIT_PORT 8501)}"
STREAMLIT_BIN="streamlit"

if [[ -x .venv/bin/streamlit ]]; then
  STREAMLIT_BIN=".venv/bin/streamlit"
fi

export STREAMLIT_BROWSER_GATHER_USAGE_STATS="${STREAMLIT_BROWSER_GATHER_USAGE_STATS:-false}"
export BACKEND_URL="${BACKEND_URL:-http://${BACKEND_HOST}:${BACKEND_PORT}}"

exec "$STREAMLIT_BIN" run ui/Home.py \
  --server.address "$STREAMLIT_HOST" \
  --server.port "$STREAMLIT_PORT"
